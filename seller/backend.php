                    ...
                    <div>
                        <table style="font-size: 13px;width: 100%;border: 1px solid #777777;">
                            ...
                            if ($_POST['type'] == "member") {
                                $pro = $obj->my_select("tbl_product",NULL,array("seller_id"=>$_SESSION['seller']))->fecth_object();
                                $trans = $obj->my_select("tbl_transaction",NULL,array("product_id"=>$pro->product_id))->fetch_object();
                                $bi = $obj->my_select("tbl_bill", NULL, array("bill_id" => $trans->bill_id))->fetch_object();
    ...
    <div class="panel-body" id="bill">
                    <div class="row"  style="margin: 45px" id="st">
                        ...
                        <div class="col-md-12">
                            ...
                            if($_POST['type'] == "date")
                            {
                                $data = $obj->my_query("SELECT b.* FROM tbl_bill as b, tbl_transaction as trans , tbl_product as pro WHERE b.bill_id = trans.bill_id AND trans.product_id = pro.product_id AND pro.seller_id = $_SESSION[seller] AND b.date = '$_POST[id]'");
                            }
                            if($_POST['type'] == "ptype")
                            {
                                $data = $obj->my_query("SELECT b.* FROM tbl_bill as b, tbl_transaction as trans, tbl_product as pro WHERE b.bill_id = trans.bill_id AND trans.product_id = pro.product_id AND pro.seller_id = $_SESSION[seller] AND b.payment_type = '$_POST[id]'");
                            }
                            if($_POST['type'] == "country")
                            {
                                $data = $obj->my_query("SELECT b.* FROM tbl_bill as b , tbl_shipment as s , tbl_location as city, tbl_location as state, tbl_location as cn, tbl_transaction as trans , tbl_product as pro WHERE b.bill_id = trans.bill_id AND trans.product_id = pro.product_id AND pro.seller_id = $_SESSION[seller] AND b.shipment_id = s.shipment_id AND city.location_id = s.city AND city.parent_id = state.location_id AND state.parent_id = cn.location_id AND cn.location_id = $_POST[id]"  );
                            }
                            if($_POST['type'] == "state")
                            {
                                $data = $obj->my_query("SELECT b.* FROM tbl_bill as b , tbl_shipment as s , tbl_location as city, tbl_location as state , tbl_transaction as trans , tbl_product as pro WHERE b.bill_id = trans.bill_id AND trans.product_id = pro.product_id AND pro.seller_id = $_SESSION[seller] AND b.shipment_id = s.shipment_id AND city.location_id = s.city AND city.parent_id = state.location_id AND state.location_id = $_POST[id]"  );
                            }
                            if($_POST['type'] == "city")
                            {
                                $data = $obj->my_query("SELECT b.* FROM tbl_bill as b , tbl_shipment as s , tbl_location as city, tbl_transaction as trans , tbl_product as pro WHERE b.bill_id = trans.bill_id AND trans.product_id = pro.product_id AND pro.seller_id = $_SESSION[seller] AND b.shipment_id = s.shipment_id AND city.location_id = s.city AND city.location_id = $_POST[id]"  );
                            }
                            if($_POST['type'] == "product")
                            {
                            ...
                            if($_POST['type'] == "price")
                            {
                                if($_POST['minprice'] != "" && $_POST['maxprice'] != "")
                                {
                                    $data = $obj->my_query("SELECT b.* FROM tbl_bill as b, tbl_transaction as trans, tbl_product as pro WHERE b.bill_id = trans.bill_id AND trans.product_id = pro.product_id AND pro.seller_id = $_SESSION[seller] AND amount >= $_POST[minprice] AND amount <= $_POST[maxprice]");
                                }
                                if($_POST['minprice'] == "" && $_POST['maxprice'] != "")
                                {
                                    $data = $obj->my_query("SELECT b.* FROM tbl_bill as b, tbl_transaction as trans, tbl_product as pro WHERE b.bill_id = trans.bill_id AND trans.product_id = pro.product_id AND pro.seller_id = $_SESSION[seller] AND amount >= 0 AND amount <= $_POST[maxprice]");
                                }
                                if($_POST['minprice'] != "" && $_POST['maxprice'] == "")
                                {
                                    $data = $obj->my_query("SELECT b.* FROM tbl_bill as b, tbl_transaction as trans, tbl_product as pro WHERE b.bill_id = trans.bill_id AND trans.product_id = pro.product_id AND pro.seller_id = $_SESSION[seller] AND amount >= $_POST[minprice]");
                                }
                            }
                                    ...
                                    <div style="border-bottom: 2px solid #999;padding: 10px">
                                        <div class="row" style="width: 98%;margin-left: 1%;">
                                            <div style="width: 50%;float: left;">
                                                ...
                                                ?>
                                                <p style="margin-top: -15px;font-size: 12px;font-weight: bold;margin-left: 5px;">E-mail : <?php echo $user->email; ?></p>
                                            </div>
                                            <div style="width: 50%;float: left;">
...
if ($action == "stock_status") {
    $status = $_POST['status'];
    if ($status == "active") {
        $aa = $obj->my_update("tbl_product", array("stock_status" => 1), array("product_id" => $id));
    }
    if ($status == "deactive") {
        $aa = $obj->my_update("tbl_product", array("stock_status" => 0), array("product_id" => $id));
    }
    ?>
    <thead>
...
if($action == "finish" || $action == "backform1")
{
    unset($_SESSION['step']);
}
if($action == "backform2")
